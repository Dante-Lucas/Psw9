...
def novo_flashcard(request):
    ...
    if request.method == "GET":
        ...
        if dificuldade_filtar:
           flashcards = flashcards.filter(dificuldade=dificuldade_filtar)   

        return render(request, 'flashcard/novo_flashcard.html', {'categorias': categorias, 'dificuldades':dificuldades, 'flashcards': flashcards})
...
def iniciar_desafio(request):
   if request.method == "GET":
      categorias = Categoria.objects.all()
      dificuldades = Flashcard.DIFICULDADE_CHOICES
      return render(request, 'flashcard/iniciar_desafio.html', {'categorias': categorias, 'dificuldades': dificuldades})
   elif request.method == 'POST':
        ...
        return redirect(reverse('desafio', args=[desafio.id]))
...
def listar_desafio(request):
    if request.method == "GET":
        ...
        return render(request,'flashcard/listar_desafio.html',{'desafios': desafios,'categorias': categorias, 'dificuldades': dificuldades})
...
def desafio(request, id):
    if request.method == 'GET':
        ...
        return render(request,'flashcard/desafio.html',{'desafio': desafio, 'acertos':acertos, 'erros':erros, 'faltantes':faltantes})
...
def relatorio(request, id):
    ...
    return render(request, 'flashcard/relatorio.html', {'desafio': desafio, 'dados': dados, 'categorias': name_categoria, 'dados2': dados2,})
